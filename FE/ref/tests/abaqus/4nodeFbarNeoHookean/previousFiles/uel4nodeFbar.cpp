...
cl /EHsc /c /Fo uel4nodeFbar.cpp /I"C:\Users\KWL\Desktop\Research\libraries\eigen-3.2.4" /I"C:\Users\KWL\Desktop\Research\libraries\boost_1_56_0"
cl /EHsc /c /Fo uel4nodeFbar.cpp 
    ...
    *ENERGY=0.0; 
	...
	for ( int lquad = 0; lquad < nquad; ++lquad )
	{
		...
		if ((nen==4)&&(nquad==4)&&(LFLAGS[1]==1)) 
		{
			...
			for ( int i = 0; i < ndm; ++i ) {
				for ( int j = 0; j < ndm; ++j ) { 
					//F1[i][j] = fac_tau*F_tau[i][j];
					//F0[i][j] = fac_t*F_t[i][j];
		...
		//fstream fs;
		//fs.open ("C++Check.dat", std::fstream::app);

		//fs << " more lorem ipsum";

		//cout << "kstep = " << *KSTEP << ", kinc = " << *KINC << ", intpt =  " << lquad << ", detmapJC = " << detjC[lquad] << ", w = " << w[lquad] << endl;
		//fs.close();
	
		/*cout << "---------------" << endl;
		cout << "Gmat at lquad = " << lquad << endl;
		for ( int i = 0; i < 4; ++i )
		{
			for ( int j = 0; j < 8; ++j )
			{
		...
		cout << "---------------" << endl;
		cout << "G0mat at lquad = " << lquad << endl;
		for ( int i = 0; i < 4; ++i )
		{
			for ( int j = 0; j < 8; ++j )
			{
					...
					for(int j=0; j<ndf; j++) { // loop over dofs of node					
						for(int I=0; I<nsdm; I++) { // loop over nsdm == ndm*ndf (tangent)
							for(int J=0; J<nsdm; J++) { // loop over nsdm == ndm*ndf (tangent)
								Kuu[a*ndf+i][b*ndf+j]+= Gmat[I][ndf*a+i]*(  Amat[I][J]*Gmat[J][ndf*b+j]  
														+ Qmat[I][J]*( G0mat[J][ndf*b+j]-Gmat[J][ndf*b+j] )  )*detjC[lquad]*w[lquad];
							}
						}
					}
				}
		...
		for(int i=0; i<ndf; i++){ // loop over dofs of node
			for(int b=0; b<nen; b++) { // loop over nodes
				for(int j=0; j<ndf; j++) { // loop over dofs of node					
					row = a*ndf+i;
					col = b*ndf+j;
					FORT_ACCESS(AMATRX,col,row,nen*ndf)=Kuu[row][col];							
				}
			}
		}
	}
...
void neohookean_constitutive_update(double * PROPS, int * NPROPS, Tensor2 & Fe_t, Tensor2 & F_t, Tensor2 & F_tau, Tensor2 & Fe_tau, Tensor2 & T_tau, Tensor4 & SpTanMod)
{
	...
	// compute right Cauchy-Green tensor C=F*F^T 
	matlib_matmult(F_tau, F_tauT, C);
	cout << "C: " << endl;
	for ( int i = 0; i < 3; ++i ) {
		for ( int J = 0; J < 3; ++J ) {
			cout << C[i][J] << " ";
	...
	// compute PK2 stress (see Bonet 2nd edition, eq 6.28, pg 163)
    for ( int I = 0; I < 3; ++I )
		for ( int J = 0; J < 3; ++J )
			S[I][J] = mu*(Iden[I][J]-Cinv[I][J])+lambda*logJ*Cinv[I][J];

	// PK1 stress P = F*S
	matlib_matmult(F_tau,S,P);
		
    // compute cauchy stress sigma = P*F^T/J
	matlib_matmult(P,F_tauT,T_tau);
	for ( int i = 0; i < 3; ++i )
		for ( int j = 0; j < 3; ++j )
			T_tau[i][j] = 0.0*T_tau[i][j]/J;
		
	// Compute C_IJKL, the so-called Lagrangian tangent modulus
	// (see Bonet 2nd edition, eq 6.30, pg 163)
	zeroT4(LagTanMod);
	coef = mu-lambda*logJ;
	for ( int I = 0; I < 3; ++I )
		for ( int J = 0; J < 3; ++J )
			for ( int K = 0; K < 3; ++K )
				for ( int L = 0; L < 3; ++L )
					LagTanMod[I][J][K][L] += lambda*Cinv[I][J]*Cinv[K][L]
									+coef*(Cinv[I][K]*Cinv[J][L]+Cinv[I][L]*Cinv[J][K]);

	// push forward to spatial								
	// (see Bonet 2nd edition, eq 6.14, pg 159)
	// actually see Ortiz lecture notes - Example 4.7.5 on Compressible Neo-Hookean
	// and Eq. 4.5.25
	// can do spatial moduli directly using Eq. 4.7.2b
	zeroT4(SpTanMod);								
    for ( int i = 0; i < 3; ++i )
		for ( int j = 0; j < 3; ++j )
			for ( int k = 0; k < 3; ++k )
				for ( int l = 0; l < 3; ++l )
					for ( int I = 0; I < 3; ++I )
						for ( int J = 0; J < 3; ++J )
							for ( int K = 0; K < 3; ++K )
								for ( int L = 0; L < 3; ++L )
									SpTanMod[i][j][k][l] += 0.0*F_tau[i][I]*F_tau[j][J]*F_tau[k][K]*F_tau[l][L]*LagTanMod[I][J][K][L]/J;	
	...
	for ( int i = 0; i < 3; ++i )
		for ( int j = 0; j < 3; ++j )
			for ( int k = 0; k < 3; ++k )
				for ( int l = 0; l < 3; ++l )
					SpTanMod[i][j][k][l] += Iden[i][k]*T_tau[j][l];*/	
