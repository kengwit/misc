...
void setup_model()
{
	...
	for (int i = 0; i < nelems; ++i ) {
		fem::element_type[i] = Element::Factory(i,CPE4); 
	}
	
	// set up two types of materials 
	fem::materials = 1;	
	fem::material_database.resize(fem::materials);
	fem::material_database[0] = Material::Factory(LINEAR_ELASTIC); 
	fem::material_database[0]->props[0] = 1.0; // lambda
	fem::material_database[0]->props[1] = 0.3; // mu
