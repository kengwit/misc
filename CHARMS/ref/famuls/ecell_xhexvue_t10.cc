...
make (GCELL *gcell)
{
  return (new ECELL_XHEXVUE_T10 (gcell));
}

bool
ECELL_XHEXVUE_T10::register_make_func ()
{
  ...
  int 
ECELL_XHEXVUE_T10::write_hexvue_hex_compressed (ckit_iobuf_t IOB,
                                  POINT p[8],
...
void 
ECELL_XHEXVUE_T10::write (ckit_iobuf_t IOB, SMART_HANDLE<QUANT_EVALUATOR>qe) 
{
  const int NFENS = 10;
  FIXED_VECTOR<3> x[15];
  vector <double> vals[15];
