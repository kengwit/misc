...
--                                                                        --
--------------------------------------------------------------------------*/
#ifndef SHAPE_APPROX_H
# define SHAPE_APPROX_H
...
class SHAPE_APPROX {
 ...
 public:
  ...
  ~SHAPE_APPROX () {
    for (typename list <FUNC_SELCRIT_PAIR *>::iterator i = _selcrits.begin(); i != _selcrits.end(); i++) {
      delete (*i);
    }
    if (_lobs) lobs_3d_free_instance (_lobs);
  }
  /**
     Apply EBC (SHAPE) to the field given as argument.
     The EBC(SHAPE_APPROX) object maintains a search structure (LOBS: Library for
  ...
  */
  ...
  /**
     Read the SHAPE_APPROXIMATION definition from a file.
  */
 ...
 private: // object functions ///////////////////////////////////////
  ...
  size_t max_level (FIELD<NUM_COMPONENTS> *src) {
    const size_t npairs = src->npairs();
    size_t maxlevel = 0;
    for (size_t j = 0; j < npairs; j++) {
      FIELD_PAIR<NUM_COMPONENTS> *fp = src->jth_field_pair (j);
      size_t level = fp->bfun()->level();
...
template <int NUM_COMPONENTS>
void
SHAPE_APPROX<NUM_COMPONENTS>::apply (FIELD<NUM_COMPONENTS> *field)
{
  const double eps = 1000000 * 2e-16; // RAW entirely arbitrary 
  size_t maxlevel = max_level (field) ;
  for (size_t level = 0; level < maxlevel+1; level++) {
    const size_t npairs = field->npairs();
    for (size_t j=0; j<npairs; j++) {
	...
	while (funpair) { // Now loop over all  selection criteria, try to match
          if (funpair->_selcrit->match (fen)) {
            ...
            //FIXED_VECTOR<NUM_COMPONENTS> v = (*funpair->_func) (ref_loc);

//****************************************************************************************************

            //Projection
        double ref_loc_norm = ref_loc.l2_norm();//ref_loc is a POINT, i.e. FIXED_VECTOR<3>
						
        //(*funpair->_func) (ref_loc) should be the radius for the 'circle' case.
        //but (*funpair->_func) (ref_loc) should be a vector
		FIXED_VECTOR<NUM_COMPONENTS> v = (*funpair->_func) (ref_loc);
		double coef = v.l2_norm()/ref_loc_norm;

		v.assign(coef, ref_loc);

//****************************************************************************************************

						
            for (size_t j=0; j < NUM_COMPONENTS; j++) { // Set constraint tags
              if (funpair->_func->given(j)) {
                fp->set_constrained_comp (j, true, v(j));
              }
            }
            FIXED_VECTOR<NUM_COMPONENTS> srcv = fp->dofparam ();
            //Now get the geometry nodal parameters for the boundary nodes.
            GCELL *gcell = bfun->gcell(0); // any gcell would do
            // Evaluate basis function values at level lower than the current level
            EVALPT evalpt (field, gcell, fen);//construct evaluation point
            evalpt.eval ();
            ...
            fp->set_dofparam_all (srcv); // Set the value of the dofparam
          }// if (funpair->_selcrit->match (fen))
...
template <int NUM_COMPONENTS>
bool
SHAPE_APPROX<NUM_COMPONENTS>::read (string file)
{
