...
GJacobi :: solve (FloatMatrix* a, FloatMatrix* b, FloatArray* eigv, FloatMatrix* x)
{
  ...
  if (! a) _error("solveYourselfAt: unknown A matrix");
  
  // and whether Bmatrix 
  if (! b) _error("solveYourselfAt: unknown Bmatrx");
  ...
  // Check output  arrays
  //
  if (!eigv) _error("solveYourselfAt: unknown eigv array");
  if (!x)    _error("solveYourselfAt: unknown x    mtrx ");
  if (eigv->giveSize() != n) _error("solveYourselfAt: eigv size mismatch");
...
# ifdef DETAILED_REPORT
    ...
    //
#ifdef DETAILED_REPORT
    printf("GJacobi: a,b after sweep\n") ;
    a->printYourself();
    b->printYourself();
#endif
    for (i = 1; i<= n; i++) {
...
# ifdef DETAILED_REPORT
    printf ("GJacobi: current eigenvalues are:\n");
    eigv->printYourself() ;
    printf ("GJacobi: current eigenvectors are:\n");
		x->printYourself();
# endif
    // 
    ...
    //
  label280:
    for (i=1; i<= n; i++) d->at(i) = eigv->at(i) ;
  } while (nsweep < nsmax) ;
