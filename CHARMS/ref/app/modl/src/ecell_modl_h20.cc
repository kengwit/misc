...
const char ECELL_MODL_H20::_type_name[] = "modl_h20";

ECELL_MODL_H20::ECELL_MODL_H20 (GCELL *gcell)
...
bool ECELL_MODL_H20::assemble_stiffness_matrix (FIELD_VECTOR *u,
                                                     EVS_OOOFS *evs, FIELD_VECTOR *geometry)
{
  GAUSS_3d_integration_rule_t *gr = gauss_rule_3d (3);
...
bool ECELL_MODL_H20::assemble_geom_stiffness_matrix (FIELD_VECTOR *u, EVS_OOOFS *evs, FIELD_VECTOR *geometry,double p)
{
...
bool ECELL_MODL_H20::assemble_geom_stiffness_matrix (FIELD_VECTOR *u, EVS_OOOFS *evs, FIELD_VECTOR *geometry)
{
  //This stiffness matrix assumes that material is homogenous isotropic
  GAUSS_3d_integration_rule_t *gr = gauss_rule_3d (3);
  vector <DOFMAPPER_VECTOR *> dv = dofmappers ();
...
bool
ECELL_MODL_H20::assem_consistent_mass (FIELD_VECTOR *u,EVS_OOOFS *evs, FIELD_VECTOR *geometry, double mmult)
{
  vector <DOFMAPPER_VECTOR *> dv = dofmappers ();
  const int nbfuns = dv.size ();
  GAUSS_3d_integration_rule_t *gr = gauss_rule_3d (3); // notice the 3x3x3 rule!
  POINT param_loc (gr->ip[0].xi, gr->ip[0].eta, gr->ip[0].theta);
...
double ECELL_MODL_H20::mass (FIELD_VECTOR *geometry) {
  double mass = 0;
  vector <DOFMAPPER_VECTOR *> dv = dofmappers ();
  const int nbfuns = dv.size ();
  GAUSS_3d_integration_rule_t *gr = gauss_rule_3d (3);
...
double
ECELL_MODL_H20::strain_energy (FIELD_VECTOR *u, FIELD_VECTOR *geometry)
{
  double sigma[3][3];
  for (int ii=0;ii<3; ii++)  for (int jj=0;jj<3; jj++) sigma[ii][jj] = 0;
...
bool
ECELL_MODL_H20::calc_stress (POINT &param_loc,FIELD_VECTOR *u, FIELD_VECTOR *geometry ,
			     double stress[6], double strain[6], double lambda, double mu)
{
...
make (GCELL *gcell)
{
  return (new ECELL_MODL_H20 (gcell));
}

bool
ECELL_MODL_H20::register_make_func ()
{
  return ECELL_MODL::register_make_func (make, string (GCELL_SOLID_H20::TYPE_NAME), string ("default"));
}
