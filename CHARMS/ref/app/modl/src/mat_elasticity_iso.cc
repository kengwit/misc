...
const string MAT_ELASTICITY_ISO::TYPE_NAME = MAT_ELASTICITY::TYPE_NAME + " " +  "elasticity_iso";

static MAT *
make (pair <DB *, string> p)
{
  return (new MAT_ELASTICITY_ISO (p.first, p.second));
}
...
MAT_ELASTICITY_ISO::register_make_func ()
{
   ...
   const char *s = MAT_ELASTICITY_ISO::TYPE_NAME.c_str();
...
MAT_ELASTICITY_ISO::MAT_ELASTICITY_ISO (DB *db, string name) : MAT_ELASTICITY (db, name)
{
  ...
  if (db->param_defined (path + "/sigma11")) {
    expr[0] = db->DB_GET_STRING (path + "/sigma11");
  }
  _sigma11 = FUNC<1> (expr);

  if (db->param_defined (path + "/sigma22")) {
    expr[0] = db->DB_GET_STRING (path + "/sigma22");
  }
  _sigma22 = FUNC<1> (expr);
  
  if (db->param_defined (path + "/sigma33")) {
    expr[0] = db->DB_GET_STRING (path + "/sigma33");
  }
  _sigma33 = FUNC<1> (expr);

  if (db->param_defined (path + "/sigma12")) {
    expr[0] = db->DB_GET_STRING (path + "/sigma12");
  }
  _sigma12 = FUNC<1> (expr);

  if (db->param_defined (path + "/sigma13")) {
    expr[0] = db->DB_GET_STRING (path + "/sigma13");
  }
  _sigma13 = FUNC<1> (expr); 
  
  if (db->param_defined (path + "/sigma32")) {
    expr[0] = db->DB_GET_STRING (path + "/sigma32");
  }
  _sigma32 = FUNC<1> (expr); 
}

bool
MAT_ELASTICITY_ISO::mat_stiffness (POINT &at, double C_mat[1][1])
{
...
bool
MAT_ELASTICITY_ISO::mat_stiffness (POINT &at, double C_mat[3][3])
{
  double lambda = _lambda (at)(0);
  double mu = _mu (at)(0);
#undef C
...
bool
MAT_ELASTICITY_ISO::mat_stiffness (POINT &at, double C_mat[6][6])
{
  double lambda = _lambda (at)(0);
  double mu = _mu (at)(0);
#undef C
