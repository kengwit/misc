...
--------------------------------------------------------------------------*/
#include "famuls.h"
#include "ecell_elasticity_l2.h"
#include "evalpt.h"
#include "square_fixed_matrix.h"
#include "gcell_line_l2.h"

const char ECELL_ELASTICITY_L2::_type_name[] = "elasticity_l2";

ECELL_ELASTICITY_L2::ECELL_ELASTICITY_L2 (GCELL *gcell)
  : ECELL_ELASTICITY::ECELL_ELASTICITY (gcell)
{
}

bool ECELL_ELASTICITY_L2::assemble_stiffness_matrix (FIELD_VECTOR *u,
...
bool ECELL_ELASTICITY_L2::assemble_tractions_load (FIELD_VECTOR *u,
                                                      LES *les, FIELD_VECTOR *geometry)
{
  ...
  if (load) {
    ...
    for (int qp = 0; qp < gr->npoints; qp++) {
      ...
      double fact = gr->ip[qp].Wgt * tangent_vector.l2_norm() / 2;
...
bool ECELL_ELASTICITY_L2::assemble_body_load (FIELD_VECTOR *u,
                                                      LES *les, FIELD_VECTOR *geometry)
{
...
bool ECELL_ELASTICITY_L2::assemble_prescribed_u_load (FIELD_VECTOR *u,
                                                      LES *les, FIELD_VECTOR *geometry)
{
  return true;
}
...
make (GCELL *gcell)
{
  return (new ECELL_ELASTICITY_L2 (gcell));
}

bool
ECELL_ELASTICITY_L2::register_make_func ()
{
  return ECELL_ELASTICITY::register_make_func (make, string (GCELL_LINE_L2::TYPE_NAME), string ("default"));
}
