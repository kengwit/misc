...
--------------------------------------------------------------------------*/
#include "famuls.h"
#include "ecell_steady_diffusion_t3_aniso.h"
#include "evalpt.h"
#include "square_fixed_matrix.h"
#include "gcell_surf_t3.h"

const char ECELL_STEADY_DIFFUSION_T3_ANISO::TYPE_NAME[] = "steady_diffusion_t3_aniso";

ECELL_STEADY_DIFFUSION_T3_ANISO::ECELL_STEADY_DIFFUSION_T3_ANISO (GCELL *gcell)
  : ECELL_STEADY_DIFFUSION::ECELL_STEADY_DIFFUSION (gcell)
{
}

bool ECELL_STEADY_DIFFUSION_T3_ANISO::assemble_conductivity_matrix (FIELD_SCALAR *phi,
...
bool ECELL_STEADY_DIFFUSION_T3_ANISO::assemble_source_terms (FIELD_SCALAR *phi,
                                                       LES *les, FIELD_VECTOR *geometry)
{
  trinumintg_integration_rule_t *tir = trinumintg_integration_rule (1);
  vector <DOFMAPPER_SCALAR *> dv = dofmappers ();
...
make (GCELL *gcell)
{
  return (new ECELL_STEADY_DIFFUSION_T3_ANISO (gcell));
}

bool
ECELL_STEADY_DIFFUSION_T3_ANISO::register_make_func ()
{
  return ECELL_STEADY_DIFFUSION::register_make_func (make, string (GCELL_SURF_T3::TYPE_NAME),
                                                     string ("aniso"));
}
