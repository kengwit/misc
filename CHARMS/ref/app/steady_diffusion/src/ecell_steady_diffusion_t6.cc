...
const char ECELL_STEADY_DIFFUSION_T6::TYPE_NAME[] = "steady_diffusion_t6";

ECELL_STEADY_DIFFUSION_T6::ECELL_STEADY_DIFFUSION_T6 (GCELL *gcell)
  : ECELL_STEADY_DIFFUSION::ECELL_STEADY_DIFFUSION (gcell)
{
}

bool ECELL_STEADY_DIFFUSION_T6::assemble_conductivity_matrix (FIELD_SCALAR *phi,
                                                              LES *les, FIELD_VECTOR *geometry)
{
  trinumintg_integration_rule_t *tir = trinumintg_integration_rule (3);
  ...
  for (int qp = 0; qp < tir->npoints; qp++) {
    POINT param_loc (tir->ip[qp].L[0], tir->ip[qp].L[1]);
...
bool ECELL_STEADY_DIFFUSION_T6::assemble_source_terms (FIELD_SCALAR *phi,
                                                       LES *les, FIELD_VECTOR *geometry)
{
  trinumintg_integration_rule_t *tir = trinumintg_integration_rule (3);
  vector <DOFMAPPER_SCALAR *> dv = dofmappers ();
...
make (GCELL *gcell)
{
  return (new ECELL_STEADY_DIFFUSION_T6 (gcell));
}

bool
ECELL_STEADY_DIFFUSION_T6::register_make_func ()
{
  return ECELL_STEADY_DIFFUSION::register_make_func (make, string (GCELL_SURF_T6::TYPE_NAME), string ("default"));
}
