...
--------------------------------------------------------------------------*/
#include "famuls.h"
#include "ecell_errest_wave_t4.h"
...
const char ECELL_ERREST_WAVE_T4::TYPE_NAME[] = "errest_wave_t4";

ECELL_ERREST_WAVE_T4::ECELL_ERREST_WAVE_T4 (GCELL *gcell)
...
void
ECELL_ERREST_WAVE_T4::assemble_error (FIELD_VECTOR *u,
                                 FIELD_VECTOR *geometry, class PROTO_ERREST<3> *proto)
{
  ...
  { // one-point quadrature
    ...
    double vol = (1./6) * evalpt.detJ (); // one-point quadrature
...
make (GCELL *gcell)
{
  return (new ECELL_ERREST_WAVE_T4 (gcell));
}

bool
ECELL_ERREST_WAVE_T4::register_make_func ()
{
  return ECELL_ERREST<3>::register_make_func (make, string (GCELL_SOLID_T4::TYPE_NAME), string ("default"));
}


void
ECELL_ERREST_WAVE_T4::attach (class PROTO_ERREST<3> *proto, string ggpath)
{
  ...
  MAT_MAP <MAT_VISCEL> mat_map (mat_mgr);
