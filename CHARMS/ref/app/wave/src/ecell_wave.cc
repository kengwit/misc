...
OFACTORY<ECELL_WAVE, ECELL_WAVE::make_ecell_func, GCELL *> * ECELL_WAVE::_ecell_factory = 0;

void
ECELL_WAVE::attach (PROTO_WAVE *proto, string ggpath)
{
  ...
  if (db->param_defined (ggpath + "/traction_load")) {
    ...
    string loadtype = db->DB_GET_STRING ("loads/" + loadname + "/type");
    //cerr << "Requesting " << loadname << "/" << loadtype << endlsizet gcell_id ;
...
bool
ECELL_WAVE::register_make_func (ECELL_WAVE::make_ecell_func make, string gcell_type, string implementation)
{
  if (_ecell_factory == 0) {
    ECELL_WAVE::_ecell_factory = new OFACTORY<ECELL_WAVE, ECELL_WAVE::make_ecell_func, GCELL* >;
  }
  return (ECELL_WAVE::_ecell_factory)->register_make_func (make, gcell_type, implementation);
}


ECELL_WAVE *
ECELL_WAVE::make_ecell (GCELL *gcell, string implementation)
{
  CHECK (ECELL_WAVE::_ecell_factory, EXCEPTION_NULL_PTR,;);
  return (ECELL_WAVE::_ecell_factory)->make (gcell, gcell->type_name (), implementation);
}
