...
--------------------------------------------------------------------------*/
#include "famuls.h"
#include "ecell_wave_traction_t3.h"
#include "evalpt.h"
#include "square_fixed_matrix.h"
#include "gcell_surf_t3.h"
#include "proto_wave.h"

const char ECELL_WAVE_TRACTION_T3::_type_name[] = "wave_traction_t3";

ECELL_WAVE_TRACTION_T3::ECELL_WAVE_TRACTION_T3 (GCELL *gcell)
...
bool 
ECELL_WAVE_TRACTION_T3::assem_tractions_load ()
{
  ...
  if (load) {
    {
      ...
      double fact = (1./2) * result.l2_norm();
...
make (GCELL *gcell)
{
  return (new ECELL_WAVE_TRACTION_T3 (gcell));
}

bool
ECELL_WAVE_TRACTION_T3::register_make_func ()
{
  return ECELL_WAVE::register_make_func (make, string (GCELL_SURF_T3::TYPE_NAME),
                                               string (_type_name));
}
