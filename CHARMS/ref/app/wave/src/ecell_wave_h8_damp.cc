...
const char ECELL_WAVE_H8_DAMP::_type_name[] = "wave_h8_damp";
const GAUSS_3d_integration_rule_t *ECELL_WAVE_H8_DAMP::_gr = gauss_rule_3d (2); // hardwired quadrature rule


ECELL_WAVE_H8_DAMP::ECELL_WAVE_H8_DAMP (GCELL *gcell)
...
bool 
ECELL_WAVE_H8_DAMP::assem_internal_forces ()
{
...
bool 
ECELL_WAVE_H8_DAMP::assem_body_load ()
{
  FIELD_VECTOR *u = _proto->u();
  FIELD_VECTOR *geometry = _proto->geometry();
...
make (GCELL *gcell)
{
  return (new ECELL_WAVE_H8_DAMP (gcell));
}

bool
ECELL_WAVE_H8_DAMP::register_make_func ()
{
  return ECELL_WAVE::register_make_func (make, string (GCELL_SOLID_H8::TYPE_NAME), string ("damp"));
}

double 
ECELL_WAVE_H8_DAMP::suggested_time_step () 
{
  FIELD_VECTOR *geometry = _proto->geometry();
...
bool 
ECELL_WAVE_H8_DAMP::assem_lumped_mass ()
{
  FIELD_VECTOR *geometry = _proto->geometry();
  FIELD_VECTOR *u = _proto->u();
...
void
ECELL_WAVE_H8_DAMP::set_watch_point_value (WATCH_POINT *watch_point, POINT &param_loc)
{
  double stress[6] = {0,0,0,0,0,0};
  double strain[6] = {0,0,0,0,0,0};
  FIELD_VECTOR *geometry = _proto->geometry();
...
FIXED_VECTOR<3> 
ECELL_WAVE_H8_DAMP::calc_loc (POINT &param_loc)
{
  FIELD_VECTOR *geometry = _proto->geometry();
...
vector<double> 
ECELL_WAVE_H8_DAMP::calc_quants (POINT &param_loc, list<string>vars) 
{
  FIELD_VECTOR *geometry = _proto->geometry();
  FIELD_VECTOR *u = _proto->u();
  FIELD_VECTOR *v = _proto->v();
...
bool 
ECELL_WAVE_H8_DAMP::calc_stress (POINT &param_loc, double stress[6], double strain[6])
{
  FIELD_VECTOR *geometry = _proto->geometry();
  FIELD_VECTOR *u = _proto->u();
...
void 
ECELL_WAVE_H8_DAMP::attach (PROTO_WAVE *proto, string ggpath)
{
  ...
  _alpha = 0;
  if (db->param_defined (path +"/alpha")) {  //rayleigh coefficient of damping 
    _alpha  = db->DB_GET_DOUBLE (path + "/alpha");
  }
...
ECELL_WAVE_H8_DAMP::~ECELL_WAVE_H8_DAMP() {
  for (int qp = 0; qp < _nqpts; qp++) { //  for each quadrature point
    delete _evalpt[qp];
  }
}
